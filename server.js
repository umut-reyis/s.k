...
let roomsData = {
    101: {
        players: [{ name: "Sistem", isHost: true, isAdmin: false, isBot: true }],
        timerVal: 15,
        timerInterval: null
    }
};
...
function handlePlayerLeave(socket) {
    if (socket.data.currentServerId) {
        ...
        if (roomsData[srvId]) {
            roomsData[srvId].players = roomsData[srvId].players.filter(p => p.socketId !== socket.id && p.name !== socket.data.playerName);
            
            // Eğer odada kalan kimse kalmadıysa ve genel lobi değilse kapat
            if (roomsData[srvId].players.length === 0 && srvId !== 101) {
                ...
                // Kurucu çıktıysa yeni kurucu ata
...
io.on('connection', (socket) => {
    ...
    socket.getServers = () => { socket.emit('server-list-update', servers); };
    ...
    socket.on('create-server', (data) => {
        ...
        roomsData[srvId] = {
            players: [{ socketId: socket.id, name: uniqueHostName, isHost: true, isAdmin: data.isAdmin, isBot: false }],
            timerVal: 15,
            timerInterval: null
        };

        // Otomatik bot doldurma (senkronize)
        let botCount = Math.floor(Math.random() * 5) + 3; // 3-7 arası bot ekle
        for(let i=0; i<botCount; i++) {
            ...
            roomsData[srvId].players.push({ name: botName, isHost: false, isAdmin: false, isBot: true });
        }
    ...
    socket.on('admin-action', (data) => {
        ...
        if (action === 'kick' || action === 'ban') {
            ...
            io.to(serverId).emit('force-disconnect', { targetName, reason: banReason });
        }
    });
